...
class CrudGenerator extends BaseCommand
{
    ...
    */
    public function handle() {
        ...
        $stub     = file_get_contents(base_path($this->location.'/Stubs/Crud.stub'));
        ...
        if(count($multiple) > 1) {
            foreach($multiple as $className) {
                ...
                $stub_fixed  = str_replace(["{{name}}","{{table}}"], [$className, $table], $stub);
                $this->makeCrud($className, $stub_fixed);
                $this->line("-----------------------------------------------");
            }
        } else {
    ...
    public function makeCrud($name, $stub) {
        ...
        $name = $name.".php";
        $path = base_path('TrusGenerator/'.$name);
