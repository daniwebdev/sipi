...
class AccessMenuController extends Controller {
    ...
    protected $view     = '.permissions.menu';
    ...
    public function index(AccessMenu $M, Request $req) {
        $model = $M;
        if(isset($req->q)) {
            $model = $M->where(DB::raw('CONCAT(title, url, route_name)'), 'LIKE', "%$req->q%");
        }

        $menus                   = $model->get()->sortByDesc('updated_at');


        $this->data['menus']     = $menus;
        $this->data['recursive'] = $M->getRecursive();
    ...
    public function delete(Request $req, AccessMenu $model, $UUID) {
        try {
            DB::beginTransaction();
            $first = $model->where('uuid', $UUID);
            $access = AccessRoleToMenu::where('access_menu_id', $first->first()->id);
